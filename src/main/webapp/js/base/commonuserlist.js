...
var addFun = function() {
	var dialog = parent.com.sirius.skymall.user.modalDialog({
		...
		url : com.sirius.skymall.user.contextPath + '/admin/base/commonUserForm.jsp',
		buttons : [ {
...
var showFun = function(id) {
	var dialog = parent.com.sirius.skymall.user.modalDialog({
		...
		url : com.sirius.skymall.user.contextPath + '/admin/base/commonUserForm.jsp?id=' + id
	});
};
...
var resetpwdFun = function(id) {
	parent.$.messager.confirm(const_message_common_info, const_message_resetpwd, function(r) {
		if (r) {
			$.post(com.sirius.skymall.user.contextPath + '/base/sycommonuser!resetpwd.sy', {
				id : id
			}, function(data) {
				alert(data.msg);
			}, 'json');
...
var removeFun = function(id) {
	parent.$.messager.confirm(const_message_common_info, const_message_role_confirmtodelete, function(r) {
		if (r) {
			$.post(com.sirius.skymall.user.contextPath + '/base/sycommonuser!delete.sy', {
				id : id
			}, function() {
	...
	$(function() {
		grid = $('#grid').datagrid({
			title : '',
			url : com.sirius.skymall.user.contextPath + '/base/sycommonuser!grid.sy',
...
// 			sortOrder : 'asc',
			...
			columns : [ [{
				...
				width : '100',
				title : const_message_userinfo_realname,
				field : 'realname'
			}, {
				width : '100',
