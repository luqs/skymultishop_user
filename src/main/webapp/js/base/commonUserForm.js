...
var submitNow = function($dialog, $grid, $pjq) {
	...
	if ($(':input[name="data.id"]').val().length > 0) {
		url = com.sirius.skymall.user.contextPath + '/base/sycommonuser!update.sy';
	} else {
...
$(function() {
//    $("#voyageid").val("30109");
//      $.post(com.sirius.skymall.user.contextPath + '/base/syvoyageinfo!getCurVoyage.sy',{},)
      $.ajax({
           type: "POST",
           url: com.sirius.skymall.user.contextPath + '/base/syvoyageinfo!doNotNeedSecurity_getCurVoyage.sy',
           dataType:'json',
           success: function(msg){
        	   $("#voyageid").val(msg.voyageId);
           }
      });
		...
		$.post(com.sirius.skymall.user.contextPath + '/base/sycommonuser!getById.sy', {
			...
			if (result.id != undefined) {
				$('form').form('load', {
					...
					'data.roomtelephone' : result.roomtelephone,
					'photo' : result.photo,
					'data.voyagId':result.voyagId
				});
				if (result.photo) {
