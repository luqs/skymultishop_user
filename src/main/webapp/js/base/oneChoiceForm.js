...
var submitNow = function($dialog, $grid, $pjq,$window) {
	...
	if ($(':input[name="data.id"]').val().length > 0) {
		url = com.sirius.skymall.user.contextPath + '/base/question!update.sy?type=4&surveyId='+const_sid;
	} else {
		url = com.sirius.skymall.user.contextPath + '/base/question!save.sy?type=4&surveyId='+const_sid;
	}
...
var appendOpts = function(){
	$(".unstyled").append('<li style=""><input  type="radio" name="radio"><input type="text" name="answers" style="width:200px;"></input><input type="checkbox" name="canInputs" value="'+i+'">是否可输入</li>');
	i++;
};
