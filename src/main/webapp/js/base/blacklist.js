var grid;
	var addFun = function() {
		var dialog = parent.com.sirius.skymall.user.modalDialog({
			title : '选择添加黑名单',
			url : com.sirius.skymall.user.contextPath + '/admin/base/blackList_add.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
...
// 			url : com.sirius.skymall.user.contextPath + '/admin/base/SyroleForm.jsp?id=' + id
			url : com.sirius.skymall.user.contextPath + '/admin/base/blacklistForm.jsp?id=' + id
		});
	};
	...
	$(function() {
		grid = $('#grid').datagrid({
			title : '',
			url : com.sirius.skymall.user.contextPath + '/base/syblacklist!grid.sy',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
