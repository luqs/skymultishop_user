...
<script src="${ctx}/js/base/publicNo.js?version=${version}" type="text/javascript" charset="utf-8"></script>
</head>
<body class="easyui-layout" data-options="fit:true,border:false">
	<div id="toolbar" style="display: none;">
		<table>
			<tr>
			    <c:if test="${appointpublicnoPermission.haveSavePermission}">
...
<%-- 				<td><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-zoom_out',plain:true" onclick="$('#searchBox').searchbox('setValue','');grid.datagrid('load',{});"><s:text name="message.role.clenquery" /></a></td> --%>
	...
	<div id="appointpublicnoAction" style="display: none;">
		<c:if test="${appointpublicnoPermission.haveUpdatePermission}">
		<img class="iconImg ext-icon-note_edit" title='<s:text name="message.publicno.revert" />' onclick="updateFun('{0}');"/>
		</c:if>
		<c:if test="${appointpublicnoPermission.haveGrantPermission}">
		<img class="iconImg ext-icon-note_edit" title='<s:text name="message.publicno.appoint" />' onclick="modifyFun('{0}');"/>
		</c:if>
	</div>
</body>
</html>